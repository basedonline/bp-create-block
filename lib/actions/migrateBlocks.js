...
function updateBlock(file) {
   ...
   /* todos
   - remove , 'blocks' from $autoloader->child(['models', 'blocks']);
   - make BlockRendererV1 to BlockRendererV2 work, or else throw error
   - add const const NAME = '${file}';
   - move $block array to block.json
   - add notice about manually updating block.json
   - prompt to continue after manually updating block.json
   */
}