...
function actionMigrateBlocks() {
   ...
   console.log(`Before you continue, make sure that all your work is committed.\n`.yellow);
...
function updateBlock(file) {
   ...
   views.forEach(file => {
      if (file.startsWith(name)) {
         const viewFile = path.resolve(viewsFolder, file);
         const dest = path.resolve(blockFolder, file);
         // if folder

         // if viewFile is a folder, copy the folder to blockFolder

         // if viewfile is a directory
         console.log(viewFile);
         if (fs.lstatSync(viewFile).isDirectory()) {
            // copy contents of folder to blockFolder
            const files = fs.readdirSync(viewFile);
            files.forEach(file => {
               const src = path.resolve(viewFile, file);
               const dest = path.resolve(blockFolder, file);
               fs.copyFileSync(src, dest);
               fs.unlinkSync(src);
            });
         } else {
            fs.copyFileSync(viewFile, dest);
            fs.unlinkSync(viewFile);
         }

         console.log(`1. Successfully moved ${file}`);
         foundFile = true;
         return;
      }
   });
   ...
   scss.forEach(file => {
      if (file.startsWith('_acf-' + name)) {
         ...
         fs.copyFileSync(src, dest);
         fs.unlinkSync(src);
         console.log(`2. Successfully moved ${file}`);
         foundFile = true;
         return;
      }
   });
   ...
   if (!content.includes(replace)) {
      ...
      console.log(`3. Successfully altering import of acf-${name}.scss`);
   }
   ...
   console.log(`4. Successfully created index.js`);
   ...
   exampleFiles.forEach(file => {
      if (file.startsWith('block.json')) {
         ...
         // find acf/example and replace it with acf/name
         var content = fs.readFileSync(src, 'utf8');
         content = content.replace(`acf/example`, `acf/${name}`);
         fs.writeFileSync(dest, content);

         console.log(`5. Successfully created block.json`);
         return;
      }
   });
   ...
   if (!content.includes('BlockRendererV2')) {
      ...
      console.log(`6. Successfully updated ${file}`);
   }
